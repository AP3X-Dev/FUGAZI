/**
 * rule-descriptions.ts — Phase 3h.4 (T197) — static map for the `explain` tool.
 *
 * Mirrors the Markdown blurbs surfaced by the CLI `explain` command
 * (`packages/cli/src/commands/explain.ts`). Mirroring (not importing) avoids a
 * dependency on the CLI package; the MCP tool surface needs to be cross-runnable
 * without the CLI's clipanion bundle.
 *
 * Keep in sync with the CLI version.
 */
